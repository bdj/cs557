...
int main(int argc, char *argv[]){
  ...
  for (;;){ // Process input lines
    ...
    else if (!strcmp(line, "pncp")) {
      int n;
      fscanf(in, "%d", &n);
      if(!nurbs[n]){
        printf("ERROR: Cannot Plot NURBS %d's Polygon. NURBS has not been initialized\n", n);
      }
      plotControlPolygon(nurbs[n]);
    }
    else if (!strcmp(line, "ikno")) {
      int n, m;
      double t;
      fscanf(in, "%d %lf %d", &n, &t, &m);
      nurbs[m] = insertKnot(nurbs[n], t);
    }
    else if (!strcmp(line, "nebz")) {
      int n, i, m;
      fscanf(in, "%d %d %d", &n, &i, &m);
      curves[m] = extractCurve(nurbs[n], i);
    }
    else if (!strcmp(line, "defo")) {
      double xmin, ymin, xmax, ymax;
      int m, n;
      fscanf(in, "%lf %lf %lf %lf %d %d", &xmin, &ymin, &xmax, &ymax, &m, &n);
      deform = new Deform(m, n, xmin, ymin, xmax, ymax);
      for (int j = 0; j <= n; j++) {
        for (int i = 0; i <= m; i++) {
          double x, y, w;
          fscanf(in, "%lf %lf %lf", &x, &y, &w);
          deform->matrix[i][j] = new Point(x * w, y * w, w);
        }
      }
    }
    else if (!strcmp(line, "grid")) {
      int nx, ny;
      fscanf(in, "%d %d", &nx, &ny);
      Curve **gridCurves = grid(deform, nx, ny);
      for (int i = 0; i < nx + ny + 2; i++) {
        plotBezier(gridCurves[i]);
      }
    }
    else if (!strcmp(line, "dcpl")) {
      int i;
      fscanf(in, "%d", &i);
      plotDeformCurve(deform, curves[i]);
    }
    else{
